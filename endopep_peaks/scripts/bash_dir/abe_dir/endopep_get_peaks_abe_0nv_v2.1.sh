...
awk -F '\t' '{print $1}' temp_4.txt > temp_5.txt
...
python ~/endopep_peaks/scripts/py_dir/df_it_n_abe.py
...
python ~/endopep_peaks/scripts/py_dir/vis_dir/stack_visual_abe.py
rm -f endo.csv

mv -f noise_reference.xlsx noise_reference_dir/noise_reference_$(date +%m.%d.%Y.%H:%M).xlsx
mv -f endopep_peak_list_n.xlsx endopep_peak_list_n_$(date +%m.%d.%Y).xlsx
...
chmod 777 endopep_peak_list_n_$(date +%m.%d.%Y).xlsx
...
if [ "$choice2"  == 'k' ] ; then
	cd ~
	mv -f ~/endopep_peaks/endopep_peak_list_n_$(date +%m.%d.%Y).xlsx ~/endopep_peaks/$choice
...
A copy of the generated noise reference output has been placed in"
tput setaf 2 ; echo "~/endopep_peaks/noise_reference_dir" ; tput sgr 0
elif [ "$choice2" = 's' ] ; then
	tput setaf 2 ; echo "
...
............"; tput sgr 0
	...
	if [ "$choice3" == $choice3 ] ; then
		cd ~
		mv -f ~/endopep_peaks/endopep_peak_list_n_$(date +%m.%d.%Y).xlsx $choice3
